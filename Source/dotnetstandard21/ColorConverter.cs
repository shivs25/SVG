    ...
    public override object ConvertTo(ITypeDescriptorContext context, CultureInfo culture, object value, Type destinationType) {
      ...
      if (value is Color) {
        ...
        if (destinationType == typeof(InstanceDescriptor)) {
          ...
          MemberInfo systemColor = null;
          try {
            systemColor = typeof(SystemColors).GetProperty(c.Name); 
          }
          catch { };

          MemberInfo knownColor = null;
          try {
            knownColor = typeof(Color).GetProperty(c.Name);
          }
          catch { }
          ...
          else if (null != systemColor) {
            member = systemColor;
          }
          else if (null != knownColor) {
            member = knownColor;
          }
          else if (c.A != 255) {
